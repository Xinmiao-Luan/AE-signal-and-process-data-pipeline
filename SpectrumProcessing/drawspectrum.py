...
def drawspectrum_py(starttime, endtime, tt, ifline=False, extralines=None, time_align=None,
    ...
    ax.set_title(f"{starttime.strftime('%H:%M:%S')} - {endtime.strftime('%H:%M:%S')}")
    ...
    if time_align is not None:
        ...
        for local_i, tmark in enumerate(visible_marks):
            ...
            if 0 < lineloc < y.shape[1]:
                ...
                if (local_i + 2) % 4 == 0:
                    ax.axvline(x=lineloc, color='red', linestyle='-', linewidth=1.5)
                else:
                    ax.axvline(x=lineloc, color='red', linestyle='--', linewidth=1)
...
# since our data gap is too long to draw:
# Loop through each 2-minute window
...
while current_start < global_end:
    ...
    try:
        drawspectrum_py(current_start, current_end, tt_list, ifline=True, time_align = time_align)
